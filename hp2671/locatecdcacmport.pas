...
(* an embedded PIC behaving like a CDC serial device. The bulk of this is       *)
(* unchanged from MS2115B hence from DSOcat and LG600, which detected and       *)
(* interfaced with a Mastech multimeter, a JYETech DSO112A oscilloscope, and a  *)
(* Logitech LG600 gamer's mouse respectively. MarkMLl.                          *)
...
(* Assuming that the OS is Linux, walk the available serial ports looking for
  a PIC acting as a CDC serial device. Note that this is also used by
  the many other embedded controllers including some Arduinos so it's not
  entirely foolproof... if it were an FTDI chip it could be branded with the
  device serial number (or model number etc.) as a one-time operation which
...
begin
//  WriteLn('In walkDirs() starting at ', dir, ' for ', pattern, ' containing ', content, ' with extra ', extra);
  result := '';
...
const
  portName= 'ttyACM';
  portidVendorDescription= '10c4';
  portidProductDescription= '8a5f';
  portDriverName= '';
...
(* investigate every port in depth it doesn't accumulate serial numbers etc.    *)
  ...
  //  DebugWriteF('Testing port %s against pattern %s\n', [portName, Backend.PortName()]);
    if Pos(portName, testPortName) = 0 then
      exit(false);
    if not usingidVendor(testPortName, portidVendorDescription, hint) then
      exit(false);
    if not usingidProduct(testPortName, portidProductDescription, hint) then
      exit(false);
  ...
  //  DebugWriteF('Testing against serial description %s\n', [Backend.PortSerialDescription()]);
    ...
    result := usingidVendor(testPortName, portidVendorDescription, hint) and result;
    result := usingidProduct(testPortName, portidProductDescription, hint) and result;
